...
export default function App() {
  ...
  const defaultMock = [
    {
      ...
      logistics: { hub: 'Ekol Lotus (Gebze)', carrier: 'Kartur Dağıtım', leadTime: '24 Saat' },
    ...
    },
    {
      ...
      logistics: { hub: 'Horoz Kapella (Tuzla)', carrier: 'Linehall Global', leadTime: '72 Saat' },
    ...
    },
    {
      ...
      logistics: { hub: 'Talay Yakut (Gebze)', carrier: 'Fillo Lojistik', leadTime: '36 Saat' },
      historicalTrend: [8, 14, 19, 22, 28, 32],
      turnoverRate: '3.9x'
    },
    {
      _id: 'inv_04',
      store: { storeCode: 'URF-BALIK-04', storeName: 'Şanlıurfa Piazza FLO', clusterSegment: 'B', city: 'Şanlıurfa', region: 'Türkiye', channel: 'Mağaza' },
      product: { sku: 'NW-BAG-440', title: 'Nine West Omuz Çantası', brand: 'Nine West', category: 'Aksesuar', unitPrice: 2199.50 },
      stockQuantity: 6,
      criticalThreshold: 15,
      predictedDemand: 30,
      suggestedReplenishment: 24,
      logistics: { hub: 'Şanlıurfa Fabrika Lojistik', carrier: 'DSD Direkt Sevk', leadTime: '12 Saat' },
    ...
    try {
      ...
      if (data.success && data.data.length > 0) {
        const enhanced = data.data.map((item, idx) => ({
          ...
          logistics: defaultMock[idx % defaultMock.length].logistics,
  ...
  const handleAutoReplenish = (item) => {
    ...
    setNotification({
      title: 'Lojistik Sevkiyat Emri Oluşturuldu (TMS)',
      message: `${item.logistics.hub} deposundan ${item.logistics.carrier} ile ${item.store.storeName} için +${qty} adet çıkış yapıldı (Teslimat: ${item.logistics.leadTime}).`
    });
  ...
  const filteredItems = inventory.filter(item => {
    const matchesSearch = item.product.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          ...
                          item.logistics.hub.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-6 font-sans">
      {/* Toast Notification */}
      {notification && (
        <div className="fixed top-5 right-5 z-50 flex items-center gap-3 bg-cyan-950 border border-cyan-500/50 text-cyan-200 px-5 py-4 rounded-xl shadow-2xl animate-bounce">
          <Truck className="text-cyan-400" size={24} />
          <div>
            <p className="font-bold text-sm text-white">{notification.title}</p>
            <p className="text-xs text-cyan-300 mt-0.5">{notification.message}</p>
          </div>
        </div>
      )}
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center pb-6 border-b border-slate-800 gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold tracking-tight text-white">Lojistik & Akıllı Tedarik Yönetim Paneli</h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">Lojistik Merkezleri (WMS), Taşıyıcı Ağları (3PL) ve Envanter Dağıtım Sistemi</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 my-6">
        <div className="bg-slate-800/80 border border-slate-700/60 p-5 rounded-xl flex items-center justify-between">
          <div>
            <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Aktif Envanter</p>
            <h3 className="text-2xl font-bold text-white mt-1">{totalStock} Adet</h3>
            <span className="text-emerald-400 text-xs mt-1 block">Depolar & Mağazalar</span>
          </div>
        ...
        <div className="bg-slate-800/80 border border-cyan-500/30 p-5 rounded-xl flex items-center justify-between">
          <div>
            <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Lojistik Merkezleri</p>
            <h3 className="text-2xl font-bold text-cyan-400 mt-1">6+ Ana Depo</h3>
            <span className="text-cyan-300 text-xs flex items-center mt-1">
              <Warehouse size={14} className="mr-0.5" /> 150.000 Palet Kapasite
            </span>
          </div>
        ...
        <div className="bg-slate-800/80 border border-amber-500/30 p-5 rounded-xl flex items-center justify-between">
          <div>
            <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Bekleyen Sevkiyat Bütçesi</p>
            ...
            <span className="text-amber-300 text-xs flex items-center mt-1">
              <CreditCard size={14} className="mr-1" /> Onaylı Satın Alma
            </span>
          </div>
        ...
        <div className="bg-slate-800/80 border border-red-500/30 p-5 rounded-xl flex items-center justify-between">
          <div>
            <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Acil Sevk İhtiyacı</p>
            <h3 className="text-2xl font-bold text-red-400 mt-1">{criticalCount} Nokta</h3>
            <span className="text-red-400 text-xs mt-1 block">Stok Kritik Eşikte</span>
          </div>
      ...
      <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="relative w-full md:w-96">
          ...
          <input 
            type="text" 
            placeholder="Ürün, mağaza veya depo ara (örn: Gebze, Şanlıurfa)..."
            value={searchTerm}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900/80 border-b border-slate-700 text-slate-400 text-xs uppercase tracking-wider">
                ...
                <th className="py-3.5 px-4 font-semibold">Hedef Mağaza</th>
                <th className="py-3.5 px-4 font-semibold">Kaynak Lojistik Deposu</th>
                <th className="py-3.5 px-4 font-semibold">Taşıyıcı & Süre</th>
                <th className="py-3.5 px-4 font-semibold text-center">Mevcut / Eşik</th>
                <th className="py-3.5 px-4 font-semibold text-center">Tahmini Talep</th>
                <th className="py-3.5 px-4 font-semibold text-center">Sevkiyat Emri</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={item._id} className="hover:bg-slate-700/30 transition">
                    ...
                    <td className="py-3 px-4">
                      ...
                      <span className="text-xs text-slate-400">{item.store.city} ({item.store.region})</span>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1.5 text-cyan-300 text-xs font-medium">
                        <Warehouse size={13} /> {item.logistics.hub}
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <span className="text-slate-200 text-xs block font-medium">{item.logistics.carrier}</span>
                      <span className="text-slate-400 text-[11px] flex items-center gap-1 mt-0.5">
                        <Clock size={11} /> {item.logistics.leadTime}
                      </span>
                    </td>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {isCritical ? (
                          <button
                            ...
                            className="inline-flex items-center gap-1 bg-cyan-600 hover:bg-cyan-500 text-white px-3 py-1.5 rounded-lg text-xs font-semibold shadow transition active:scale-95"
                          >
                            <Send size={13} />
                            +{item.suggestedReplenishment} Sevk Et
                          </button>
                        ) : (
                          <span className="inline-flex items-center gap-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 px-2.5 py-1 rounded-full text-xs font-semibold">
                            <CheckCircle2 size={12} /> Stok Yeterli
                          </span>
                        )}
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-lg p-6 shadow-2xl relative">
            ...
            <div className="flex items-center gap-2 text-orange-400 mb-1">
              <BarChart3 size={18} />
              <h3 className="text-xs font-bold uppercase tracking-wider">Talep ve Sevkiyat Analizi</h3>
            </div>
            <h2 className="text-lg font-bold text-white">{selectedProduct.product.title}</h2>
            <p className="text-xs text-slate-400 mt-0.5">{selectedProduct.store.storeName} — Kaynak: {selectedProduct.logistics.hub}</p>
            ...
            <div className="mt-6 bg-slate-900/80 border border-slate-800 p-4 rounded-xl">
              <p className="text-xs text-slate-400 mb-3 flex items-center justify-between">
                <span>Son 6 Aylık Satış / Çıkış Hacmi</span>
                ...
                {selectedProduct.historicalTrend.map((val, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end">
                    ...
                    <div 
                      ...
                      className="w-full bg-gradient-to-t from-cyan-600 to-teal-400 rounded-t transition-all duration-500"
                    />
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              <div className="bg-slate-900/50 p-3 rounded-xl border border-slate-800">
                <span className="text-[11px] text-slate-400 block">Atanan Taşıyıcı</span>
                <span className="text-sm font-bold text-cyan-300 mt-0.5 block">{selectedProduct.logistics.carrier}</span>
              </div>
              <div className="bg-slate-900/50 p-3 rounded-xl border border-slate-800">
                <span className="text-[11px] text-slate-400 block">Tahmini Teslim Süresi</span>
                <span className="text-sm font-bold text-emerald-400 mt-0.5 block">{selectedProduct.logistics.leadTime}</span>
              </div>
            </div>
            ...
            <div className="mt-5 flex justify-end gap-2">
              {selectedProduct.stockQuantity <= selectedProduct.criticalThreshold && (
                <button
                  ...
                  className="w-full inline-flex items-center justify-center gap-2 bg-cyan-600 hover:bg-cyan-500 text-white py-2.5 rounded-xl text-xs font-semibold shadow transition active:scale-95"
                >
                  <Send size={14} />
                  +{selectedProduct.suggestedReplenishment} Adet Sevkiyatı Başlat
                </button>
              )}
            </div>
          </div>
